...
export default function ForgotPassword() {
  ...
  const handleReset = async(e) => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_SERVER_URL}/users/reset-password`, { email })
      if(res.status === 200) {
        setLoading(false)
  ...
  const handleNewPassword = async(e) => {
    ...
    try {
      ...
      const res = await axios.post(`${process.env.REACT_APP_SERVER_URL}/users/new-password`, { email, newPassword })
      if(res.status === 200) {
        setLoading(false)
